...
{
    std::cout << "FR4G-TP <" << Name << "> This time it'll be awesome, I promise!" << std::endl;
}

FragTrap::~FragTrap()
{
    std::cout << "FR4G-TP <" << Name << "> Oh darn, oh boy, oh crap, oh boy, oh darn." << std::endl;
}
...
void FragTrap::vaulthunter_dot_exe(std::string const & target)
{
    if (Energypoints >= 25)
    {
        ...
        std::cout << '<' << Name << "> attacks <" << target << "> with a <" << attacks[std::rand()%5] << "> causing <" << damage << "> points of damage" << std::endl;
    }
    else 
    {
        std::cout << '<' << Name << "> is out of energy! What a shame..." << std::endl;
    }
}